  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success || response.statusCode === 200) {
        ...
        // Redirect to home with hard reload to reinitialize AuthContext
    ...
    <>
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="bg-white shadow-md rounded-xl w-full max-w-md p-8 space-y-6">
          ...
          {message && (
            <div
              className={`text-center mt-4 text-sm font-medium ${messageType === 'success' ? 'text-green-600' : 'text-red-600'
                }`}
            >
              {message}
            </div>
